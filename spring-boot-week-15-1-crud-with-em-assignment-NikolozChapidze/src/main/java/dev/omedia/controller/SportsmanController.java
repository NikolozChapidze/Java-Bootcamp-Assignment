...
public class SportsmanController {
    ...
    @DeleteMapping("{id}")
    public int removeSportsman(@PathVariable long id) {
        return sportsmanService.removeSportsman(id);
    }

    @PostMapping
    public void addSportsman(@RequestBody Sportsman sportsman) {
        long teamId = sportsman.getTeam().getId();
        if (teamNotExists(teamId)) {
    ...
    @PutMapping("{id}")
    public Sportsman updateSportsman(@PathVariable long id, @RequestBody Sportsman sportsman) {
        long teamId = sportsman.getTeam().getId();
        if (teamNotExists(teamId)) {
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Team Not Found with id " + teamId);
        }
    ...
    public boolean teamNotExists(long teamId) {
        return teamService.getTeam(teamId).isEmpty();
    }
}
